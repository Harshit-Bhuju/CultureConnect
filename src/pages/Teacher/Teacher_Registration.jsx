...
import { Upload, X, Check, ArrowLeft, Clock, Mail } from "lucide-react";
...
function TeacherRegistration() {
  ...
  const handleImageSelect = (e) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      toast.error("Profile picture must be ≤ 4MB");
      return;
    }
  ...
  }, [formData, profileFile, certificates]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (profileFile) body.append("profilePicture", profileFile);

      certificates.forEach((cert) => {
        body.append("certificates[]", cert);
      });
            ...
            >
              ...
              <ul className="space-y-2 text-sm text-gray-700 mb-4">
                <li className="p-3 bg-white rounded-lg border">
                  • Teachers must provide accurate qualifications
                </li>
                <li className="p-3 bg-white rounded-lg border">
                  • Classes must follow CultureConnect community guidelines
                </li>
                <li className="p-3 bg-white rounded-lg border">
                  • Payments are settled after successful class completion
                </li>
                <li className="p-3 bg-white rounded-lg border">
                  • Platform commission applies per session
                </li>
              </ul>
