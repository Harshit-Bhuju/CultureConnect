// src/components/seller/SellerReviewsView.jsx
import React, { useState } from "react";
import { Star, StarHalf, MessageCircle, Reply, Send, X, Edit as EditIcon, Trash2, Store, Check } from "lucide-react";
import API, { BASE_URL } from "../../../../Configs/ApiEndpoints";
import { useNavigate } from "react-router-dom";
import DeleteReviewModal from "./DeleteReviewModal";

const SellerReviewsView = ({ product, onSubmitReply, onDeleteReply, sellerId }) => {
  ...
  // Update local reviews when product prop changes
  React.useEffect(() => {
    if (product?.reviews) {
  ...
  if (!product) {
    return (
      <div className="text-center py-16 bg-yellow-50 rounded-lg border-2 border-yellow-200">
        ...
        <p className="text-yellow-700 text-xl font-semibold mb-2">⚠️ No Product Data</p>
        <p className="text-yellow-600">Product object is null or undefined. Check parent component.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-200">
        <div className="flex items-center gap-4">
          ...
          <div className="border-l-2 border-blue-300 pl-4">
            ...
            <p className="text-sm text-gray-600 mt-1">Customer feedback for your product</p>
          </div>
        </div>
      </div>
      ...
      {sortedReviews.length > 0 ? (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="font-semibold text-gray-900 text-lg">{review.author || 'Anonymous'}</p>
                ...
                {/* Review Content */}
                <p className="text-gray-700 mb-5 leading-relaxed">{review.comment || 'No comment provided'}</p>

                {/* Seller Reply Display/Edit Section */}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {sellerReply.storeName || 'You'}
                            </p>
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-medium rounded-full">
                              Seller
                            </span>
                            <span className="text-xs text-gray-500">
                              {formatDate(sellerReply.createdAt)}
                              {sellerReply.updatedAt && sellerReply.updatedAt !== sellerReply.createdAt && " • Edited"}
                            </span>
                          </div>
                        </div>
                        <p className="text-gray-700 text-sm leading-relaxed ml-9">{sellerReply.replyText}</p>
                      </div>
                      <div className="flex gap-1 ml-3">
                        <button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            handleEditReply(sellerReply, review.id);
                          }}
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            handleDeleteClick(sellerReply.id, review.id);
                          }}
                ...
                {isEditingThisReply && (
                  <div className="mt-4 border-l-4 border-blue-500 bg-gradient-to-r from-blue-50 to-transparent rounded-r-lg p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-semibold text-blue-900">Editing your reply</span>
                    </div>
                    <textarea
                      value={replyText}
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <p className="text-sm text-gray-600">
                        ...
                        {replyText.length < 10 && replyText.length > 0 && (
                          <span className="text-red-500 ml-2 font-medium">• Min 10 characters required</span>
                        )}
                      </p>
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          disabled={!replyText.trim() || replyText.trim().length < 10 || isSubmitting}
                ...
                {!hasReply && !isReplying ? (
                  ...
                  <div className="mt-4 border-l-4 border-green-500 bg-gradient-to-r from-green-50 to-transparent rounded-r-lg p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-semibold text-green-900">Writing your reply</span>
                    </div>
                    <textarea
                      value={replyText}
                      onChange={(e) => setReplyText(e.target.value)}
        ...
        <div className="text-center py-16 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-500 text-xl font-semibold mb-2">No Reviews Yet</p>
          <p className="text-gray-400">Your product hasn't received any customer reviews yet.</p>
        </div>
      )}
